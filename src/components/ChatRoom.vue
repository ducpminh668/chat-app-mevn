    ...
    <b-col cols="12">
      ...
      <b-form @submit="onSubmit" class="chat-form">
        <b-input-group prepend="Message">
          <b-form-input id="message" :state="true" v-model.trim="chat.message"></b-form-input>
...
export default {
  ...
  created() {
    axios.get(`http://localhost:3000/api/chat/${this.$route.params.id}`)
      .then((response) => {
        this.chats = response.data ? response.data : [];
      })
      .catch((e) => {
        this.errors.push(e);
      });
    this.socket.on('new-message', this.pushMessages);
  },
  methods: {
    pushMessages(data) {
